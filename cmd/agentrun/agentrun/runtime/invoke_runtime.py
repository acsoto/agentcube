...
class InvokeRuntime:
    ...
    def get_agent_status(self, workspace_path: Path, use_k8s: Optional[bool] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if this is a K8s deployment
            check_k8s = use_k8s if use_k8s is not None else (self.use_k8s or metadata.k8s_deployment is not None)

            if check_k8s:
                # Get status from K8s cluster
                return self._get_k8s_status(metadata)
            else:
                # For AgentCube, return status from metadata
                return {
                    "agent_id": metadata.agent_id,
